module.exports = function(config){
    ...
    if(!global.cloud_mysql_pools){
        global.cloud_mysql_pools = {};
        process.on('exit', function () {
            console.log("connection pool released.");
            for(var dbName in global.cloud_mysql_pools){
                var poolCluster = global.cloud_mysql_pools[dbName];
                poolCluster.end();
                delete global.cloud_mysql_pools[dbName];
            }
            delete global.cloud_mysql_pools;
        });
    }

    var GetPool = function(dbName, wr){
        var poolCluster
        poolCluster = global.cloud_mysql_pools[dbName];
        ...
        if(!poolCluster){
            ...
            global.cloud_mysql_pools[dbName]  = poolCluster;
//            if(config[dbName].connectionLimit){
//                global.cloud_mysql_pools[dbName].connectionLimit = config[dbName].connectionLimit;
//            }
        }
